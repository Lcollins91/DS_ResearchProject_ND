...
dispersion1 = [0.439, 0.4068, -10.996];
eps = 0.00001;
%ft_sim = fittype('A*sin(2*k*x+phi)', 'problem', 'k', 'independent', 'x');
...
for i = 1:training_size
    ...
    for j = 1:9
        ...
        [fit_sim_3, good_sim_3] = fit(x_sim(40:end)', training1{i,2}(j,40:end)', ft3a, 'problem', k_sim(j),'StartPoint', [-0.2,-0.2],  'Lower', [-0.5, -0.5], 'Upper', [0.5, 0.5]);
        ...
        training2(((i-1)*9+j),:) = [fit_sim_3.a fit_sim_3.b deltaI deltaR E(j) k_sim(j) peaks_temp locs_temp width_temp prom_temp];
%         if j == 5
%             figure; scatter(x_sim', training1{i,2}(j,:))
%             hold on
%             scatter(x_sim(40:end)', training1{i,2}(j,40:end), 'k')
%             plot(fit_sim_3,'g')
%             plot(fit_sim, 'm')
%             ylim([-0.5,0.5])
%         end
...
save('/Users/lauracollins/Desktop/DS_ResearchProject_ND/LineCutTrainingData052118_fixed.mat', 'training2');
csvwrite('/Users/lauracollins/Desktop/DS_ResearchProject_ND/LineCutTrainingData052118_fixedfit.csv', training2);
...
% loading the data
data = k3ds('GridSpec001.3ds');

% Plot of topo and all didv maps
figure; 
subplot(1,10,1)
topo=ltstrip(data.Topo,3);
imagesc(topo); axis image xy;
title('Topo')
data.LIX=(data.LIXf+data.LIXb)/2;
for i= 1:9
    specs(:,:,i)=data.LIX(:,:,i)/mean2(data.LIX(1:100,:,i))-1;
    subplot(1,10,i+1);
    imagesc(specs(:,:,i));
    caxis([-.2,.2]);axis image xy off;
    title(['E= ' num2str(-.5+i*.1) ' V'])
end
kcm('gold');

% Avereging each line cut in space
...
% Number of peaks that we write on output table
nkeep = 5;
